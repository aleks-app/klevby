(function () {
  ...
  async function deleteMessage(type, id) {
    ...
    if (!liveId) { console.error("[KlevbyDelete] missing message id", { type: liveType, id: liveId }); alert("Нельзя удалить: у сообщения отсутствует id."); cleanupMenuState("delete_missing_id"); return; }
    if (liveIsMine === false) { console.error("[KlevbyDelete] blocked delete for чужое сообщение", { id: liveId, type: liveType }); cleanupMenuState("delete_not_mine"); return; }
    const elements=getElements(); const messagesContainer=elements.messagesContainer||null; if(!confirm("Удалить сообщение?")) return; await refreshCurrentUser({force:true}); const currentChatUser=getCurrentUser(); const client=getMainSupabaseClient(); if(!client||typeof client.from!=="function"){alert("Нет подключения к Supabase."); return;} let result; const currentChatUserId=currentChatUser?.id||null; const currentChatUserIdValid=isValidSupabaseUuid(currentChatUserId); let deletePath="public-user-name-fallback"; if(liveType==="private"){ deletePath="private"; } else if(currentChatUserIdValid){ deletePath="public-user-id"; } console.info("[KlevbyDelete] delete request",{liveType,liveId,liveIsMine,currentChatUserId,currentChatUserIdValid,currentChatName:getCurrentChatName(),deletePath}); if(liveType==="private"){ if(!currentChatUser||!currentChatUserIdValid){alert("Удалять личные сообщения можно только после входа."); return;} result=await client.from("private_messages").delete().eq("id",liveId).eq("sender_id",currentChatUser.id);} else { if(currentChatUser&&currentChatUserIdValid){ result=await client.from("messages").delete().eq("id",liveId).eq("user_id",currentChatUser.id);} else { result=await client.from("messages").delete().eq("id",liveId).eq("user_name",getCurrentChatName()); } } if(result.error){console.error("[KlevbyDelete] delete failed",{id:liveId,type:liveType,isMine:liveIsMine,deletePath,error:result.error}); alert(`Не получилось удалить сообщение: ${result.error.message||"ошибка RLS/бэкенда"}`); return;} console.info("[KlevbyDelete] delete success",{id:liveId,type:liveType}); if(messagesContainer){ const row=messagesContainer.querySelector(`[data-message-id="${cssEscape(liveId)}"][data-message-type="${liveType}"]`); if(row){ row.remove(); } } cleanupMenuState("delete_success"); }

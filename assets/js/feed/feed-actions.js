(function () {
  ...
  function getCoreHelper(name) {
    const core = getCore();
    const helper = core && core[name];

    return typeof helper === "function" ? helper : null;
  }

  function getState() {
    return getCoreHelper("getState")?.() || window.KlevbyFeedState || {};
  }

  function getApi() {
    return getCoreHelper("getApi")?.() || window.KlevbyFeedApi || {};
  }

  function getRender() {
    return getCoreHelper("getRender")?.() || window.KlevbyFeedRender || {};
  }

  function getModals() {
    return getCoreHelper("getModals")?.() || window.KlevbyFeedModals || {};
  }

  function getUtils() {
    return getCoreHelper("getUtils")?.() || window.KlevbyFeedUtils || {};
  }

  function withSoftTimeout(promise, timeoutMs, fallbackValue = null, label = "operation") {
    const helper = getCoreHelper("withSoftTimeout");

    if (helper) {
      return helper(promise, timeoutMs, fallbackValue, label);
    }
  ...
  function getSupabaseClient() {
    const helper = getCoreHelper("getSupabaseClient");

    return helper ? helper() : null;
  }

  function getSupabaseUrl() {
    const helper = getCoreHelper("getSupabaseUrl");

    return helper ? helper() : "";
  }

  function getSupabaseAnonKey() {
    const helper = getCoreHelper("getSupabaseAnonKey");

    return helper ? helper() : "";
  }

  function getCurrentUser() {
    const helper = getCoreHelper("getCurrentUser");

    return helper ? helper() : null;
  }

  function isPromiseLike(value) {
    const helper = getCoreHelper("isPromiseLike");

    return helper ? helper(value) : Boolean(value && typeof value.then === "function");
  }

  function isMobileLikeViewport() {
    const helper = getCoreHelper("isMobileLikeViewport");

    return helper ? helper() : true;
  }

  function rememberViewerUserId(userId) {
    const helper = getCoreHelper("rememberViewerUserId");
  ...
  function getKnownViewerUserIdSync() {
    const helper = getCoreHelper("getKnownViewerUserIdSync");

    return helper ? helper() : "";
  }

  function readStoredSupabaseAccessToken() {
    const helper = getCoreHelper("readStoredSupabaseAccessToken");

    return helper ? helper() : "";
  }

  async function getSupabaseAccessToken() {
    const helper = getCoreHelper("getSupabaseAccessToken");

    return helper ? helper() : "";
  }

  async function ensureCurrentUser() {
    const helper = getCoreHelper("ensureCurrentUser");

    return helper ? helper({ timeoutMs: LIKE_READ_TIMEOUT_MS }) : null;
  }
  ...
  function isHomeVisible() {
    const helper = getCoreHelper("isHomeVisible");
  ...
  function getLastItemsArray() {
    const helper = getCoreHelper("getLastItemsArray");

    return helper ? helper() : [];
  }

  function setLastItemsArray(items) {
    const helper = getCoreHelper("setLastItemsArray");
  ...
  function getCachedFeedItem(postId) {
    const helper = getCoreHelper("getCachedFeedItem");

    return helper ? helper(postId) : null;
  }

  function getItemLikesCount(item) {
    const helper = getCoreHelper("getItemLikesCount");

    return helper ? helper(item) : 0;
  }

  function getKnownLikeStateFromItem(item) {
    const helper = getCoreHelper("getKnownLikeStateFromItem");
  ...
  function extractPostIdFromDetail(detail = {}) {
    const helper = getCoreHelper("extractPostIdFromDetail");

    return helper ? helper(detail) : "";
  }

  function isLikeUpdateDetail(detail = {}) {
    const helper = getCoreHelper("isFeedLikeUpdateDetail");

    return helper ? helper(detail) : false;
  }
  ...
  function patchLocalFeedItem(postId, patch = {}) {
    const helper = getCoreHelper("patchLocalFeedItem");

    return helper ? helper(postId, patch) : null;
  }

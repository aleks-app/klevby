(function () {
  ...
  const MARKET_ALLOWED_TYPES = ["image/jpeg", "image/png", "image/webp"];
  ...
  async function compressMarketImageIfPossible(file) {
    ...
    try {
      ...
      console.warn("Klevby барахолка: компрессия фото пропущена:", error);
      return file;
    }
  }
  ...
  async function uploadMarketPhotoFile(params) {
    ...
    if (!MARKET_ALLOWED_TYPES.includes(file.type)) throw new Error("MARKET_UPLOAD_TYPE_INVALID");
    ...
    if (!MARKET_ALLOWED_TYPES.includes(preparedFile.type)) throw new Error("MARKET_UPLOAD_TYPE_INVALID");
  ...
  window.KlevbyMarket.MARKET_ALLOWED_TYPES = MARKET_ALLOWED_TYPES.slice();
})();

(function () {
  ...
  function isLikelyHeicOrHeif(file) {
  ...
  async function prepareMarketPhotoFile(file) {
    if (typeof document === "undefined") return file;
    if (typeof HTMLCanvasElement === "undefined") return file;

    try {
      if (isLikelyHeicOrHeif(file)) throw new Error("MARKET_HEIC_UNSUPPORTED");
      ...
      if (!blob) throw new Error("MARKET_IMAGE_PROCESS_FAILED");
      let preparedFile = file;
      if (blob.size < file.size || sourceWidth > MARKET_UPLOAD_MAX_DIMENSION || sourceHeight > MARKET_UPLOAD_MAX_DIMENSION) {
        preparedFile = new File([blob], file.name.replace(/\.[a-zA-Z0-9]+$/, "") + "." + getOutputExtension(outputType), {
          type: outputType,
      ...
      if (code.includes("MARKET_HEIC_UNSUPPORTED")) throw error;
      if (code.includes("MARKET_UPLOAD_TOO_LARGE") || code.includes("MARKET_UPLOAD_TYPE_INVALID")) throw error;
  ...
  window.KlevbyMarket.prepareMarketPhotoFile = prepareMarketPhotoFile;
})();

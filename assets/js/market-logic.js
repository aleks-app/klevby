(function () {
  ...
  function isTitlePrefixSearchMatch(searchValue, titleValue) {
    ...
    const titleStems = titleTokens.map(normalizeRuTokenStem);

    return searchTokens.every(function (searchToken) {
      const searchStem = normalizeRuTokenStem(searchToken);
      if (!searchStem || searchStem.length < 3) return false;

      return titleTokens.some(function (titleToken, index) {
        if (titleToken.startsWith(searchStem)) return true;
        const titleStem = titleStems[index] || "";
        return titleStem.startsWith(searchStem) || searchStem === titleStem;
      });
    });
  }
  ...
  function isMarketItemSearchMatch(item, search) {
    ...
    if (categoryRule) {
      ...
      if (categoryRule.matchType === "title") {
        return isMarketItemInSearchCategory(item, categoryRule) && isTitlePrefixSearchMatch(search, item.title);
      }

(function () {
  ...
  function marketDetailsHtml(item) {
    ...
    const renewBtn = canManage
      ? `<button class="small-btn green" type="button" onclick="applyMarketOwnerAction('${safeId}', 'renew')">Продлить на 30 дней</button>`
      : "";
    const soldBtn = canManage
      ? `<button class="small-btn gray" type="button" onclick="applyMarketOwnerAction('${safeId}', 'sold')">Отметить продано</button>`
      : "";
  ...
  async function applyMarketOwnerAction(id, action) {
    ...
    if (action === "renew") {
      ...
      successMessage = "Объявление продлено на 30 дней.";
    } else if (action === "sold") {
      payload = {
        status: "sold",
        sold_at: nowIso
    ...
    try {
      await loadMarketItems({ force: true });
      if (marketOpenDetailsItemId && String(marketOpenDetailsItemId) === String(id)) {
        const refreshedItem = marketItems.find(function (x) {
          return String(x.id) === String(id);
        });
        if (refreshedItem) openMarketItemDetails(id);
        else closeMarketItemDetails();
      }
    } catch (error) {

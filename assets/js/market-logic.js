  ...
  async function getMarketAccessToken() {
    ...
    try {
      if (marketDb?.auth?.getSession) {
        ...
        const tokenFromSdk = sessionResult?.data?.session?.access_token || "";
        if (tokenFromSdk) return String(tokenFromSdk);
      }
    } catch (error) {
    ...
    const tokenFromStorage = storedSession?.access_token || "";
    if (tokenFromStorage) return String(tokenFromStorage);

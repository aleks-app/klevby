...
async function initAuth() {
  ...
  if (!currentUser && restorePendingSignup()) {
    window.klevbyAuthStatusNotice = "Введите 6-значный код из последнего письма или отправьте код ещё раз.";
    setAuthMode("verify");
  } else {
...
async function register() {
  ...
  if (!activeSession) {
    ...
    window.klevbyAuthStatusNotice = "Код отправлен на почту. Введите 6-значный код из письма.";
    setAuthMode("verify");
    updateAuthStatus();
    reloadPondsIfReady();
    return;
...
async function verifySignupCode() {
  ...
  const token = String(document.getElementById("authCodeInput")?.value || "").trim();
  ...
  if (!/^\d{6}$/.test(token)) {
    window.klevbyAuthStatusNotice = "Введите 6-значный код из письма.";
    updateAuthStatus();
    return;
  }

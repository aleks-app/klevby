(function () {
  ...
  async function loadPublicProfilePhotos(userId) {
    ...
    if (!cleanId) return { status: "error", photos: [] };

    const feedStateItems = getFeedStateItems();
    if (feedStateItems.length) {
      const cached = feedStateItems.filter((item) => String(item?.userId || item?.user_id || "") === cleanId && String(item?.type || "") === "profile_photo");
      if (cached.length) {
        return { status: "ok", photos: cached };
      }
    }
    ...
    if (supabaseUrl && supabaseAnonKey) {
      try {
        ...
        params.set("user_id", `eq.${cleanId}`);
        params.set("type", "eq.profile_photo");
        ...
        if (response.ok) {
          ...
          return { status: "ok", photos: Array.isArray(data) ? data : [] };
        }
      } catch (_) {}
    }

    const client = getFeedClient();
    if (!client) return { status: "error", photos: [] };
    try {
      const { data, error } = await client
        .from("feed_posts")
  ...
  async function openKlevbyPublicProfile(userId, fallbackData = {}) {
    ...
    const photosResult = await withTimeout(loadPublicProfilePhotos(cleanId), 5000, { status: "timeout", photos: [] });
    if (state.requestToken !== requestToken || state.viewedUserId !== cleanId) return;

(function () {
  ...
  function getPhotosFromFeedState(userId) {
    const cleanUserId = String(userId || "").trim();
    if (!cleanUserId) return [];

    const feedState = window.KlevbyFeedState || {};
    if (typeof feedState.getLastItems !== "function") return [];
    ...
    items.forEach((item) => {
      ...
      if (!itemUserId || itemUserId !== cleanUserId) return;

      const id = String(item?.id || "").trim();
      const imageUrl = String(item?.image || item?.imageUrl || item?.image_url || "").trim();
      const imagePath = String(item?.imagePath || item?.image_path || "").trim();

      if (!imageUrl && !imagePath) return;

      const dedupeKey = id || imageUrl || imagePath;
  ...
  async function getPublicProfilePhotos(userId) {
    const cleanUserId = String(userId || "").trim();
    if (!cleanUserId) return [];
    const feedStatePhotos = getPhotosFromFeedState(cleanUserId);

    const supabase = getSupabaseClient();
    if (!supabase) return feedStatePhotos;
    ...
    try {
      ...
      if (error) {
        ...
        return feedStatePhotos;
      }

      const supabasePhotos = Array.isArray(data)
        ? data.map(mapFeedPhoto).filter((item) => item.id && item.image_url)
        : [];

      if (supabasePhotos.length) {

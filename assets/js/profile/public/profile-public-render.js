(function () {
  ...
  function render({ profile, posts, loading, error }) {
    ...
    if (error) {
      root.innerHTML = `<div class="klevby-public-profile-panel"><div class="klevby-public-topbar"><div class="klevby-public-topbar"><button class="klevby-public-back" type="button" onclick="window.closeKlevbyPublicProfile?.()">← Назад</button></div></div><div class="klevby-public-empty"><strong>Не удалось открыть профиль.</strong><div class="klevby-public-error-text">${esc(error)}</div><button class="klevby-public-return-btn" type="button" onclick="window.closeKlevbyPublicProfile?.()">Вернуться в ленту</button></div></div>`;
      return;
    }

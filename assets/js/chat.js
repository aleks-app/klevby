(function () {
  ...
  function setupChat(chatDb) {
      ...
      <div id="klevby-chat-modal" class="hidden">
        <div id="chat-window" class="klevby-chat-window">
          <div id="chat-header" class="klevby-chat-header">
            ...
            <button id="klevby-chat-call" class="klevby-chat-call hidden" type="button" aria-label="Позвонить">📞</button>
    ...
    const callBtn = document.getElementById("klevby-chat-call");
    ...
    function updateCallButtonVisibility() {
      ...
      callBtn.classList.toggle("hidden", !shouldShow);
    }
    ...
    function startCallTimer() {
      ...
      const timerEl = document.getElementById("klevbyCallTimer");
      if (timerEl) timerEl.textContent = "00:00";

      callTimer = setInterval(() => {
        callSeconds += 1;
        const el = document.getElementById("klevbyCallTimer");
    ...
    function showCallToast(text) {
      const oldToast = document.getElementById("klevbyCallToast");
    ...
    function closeCallOverlay(showToast = true) {
      ...
      const overlay = document.getElementById("klevbyCallOverlay");
      if (overlay) overlay.remove();

      callOverlay = null;

      const chatIsOpen = modal && modal.classList.contains("open");
      ...
      if (showToast) {
        showCallToast("Вызов завершён");
      }
    }

    function openCallOverlay() {
      if (!(activeMode === "private" && selectedPeer && selectedPeer.id)) {
        return;
      }

      closeCallOverlay(false);

      const peerName = selectedPeer.name || "Собеседник";
      ...
      callOverlay.innerHTML = `
        <div class="klevby-call-card">
          <div class="klevby-call-avatar-wrap">
            ...
            <div class="klevby-call-pulse"></div>
            <div class="klevby-call-avatar">${escapeHtml(getInitials(peerName))}</div>
          </div>
    ...
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape" && document.getElementById("klevbyCallOverlay")) {
    ...
    document.addEventListener("click", async (event) => {
      ...
      if (event.target.closest("#klevby-chat-call")) {
        event.preventDefault();
        event.stopPropagation();
        openCallOverlay();
        return;
      ...
      if (event.target.closest("#klevbyEndCallBtn")) {
        event.preventDefault();
        event.stopPropagation();
        closeCallOverlay(true);
        return;
  ...
  function injectExtraChatStyles() {
    ...
    style.textContent = `
      ...
      .klevby-chat-call {
        ...
        box-shadow: 0 0 20px rgba(87,230,178,0.16) !important;
      }

      .klevby-chat-call:active,
